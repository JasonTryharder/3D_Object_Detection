...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            labels_corners = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length, label.box.heading)

            ## step 2 : loop over all detected objects
            for d in detections:
                ## step 3 : extract the four corners of the current detection
                cl_id, x, y, z, h, w, l, yaw = d
                det_corners = tools.compute_box_corners(x, y, w, l, yaw)
                
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                d_x = label.box.center_x - x
                d_y = label.box.center_y - y
                d_z = label.box.center_z - z
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                p1 = Polygon(labels_corners)
                p2 = Polygon(det_corners)
                ...
                if iou > min_iou:
                    matches_lab_det.append([iou, d_x, d_y, d_z])
                    true_positives += 1
    ...
    all_positives = np.sum(np.where(labels_valid, labels_valid, 0)) # all items indicated positive in the solution label

    ## step 2 : compute the number of false negatives
    false_negatives = all_positives - true_positives # all indicated as positives - items detected as positives

    ## step 3 : compute the number of false positives
    false_positives = len(detections) - true_positives # items actually detected with iou < threshold
...
def compute_performance_stats(det_performance_all, configs):
    ...
    all_positives, true_positives, false_negatives, false_positives = np.sum(pos_negs[:,0]), np.sum(pos_negs[:,1]), np.sum(pos_negs[:,2]), np.sum(pos_negs[:,3])
